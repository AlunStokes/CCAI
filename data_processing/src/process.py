    ...
    with open(os.path.join(input_path), 'r') as csv_file:
        ...
        for row in csv_reader:
            if line_count == 0:
                ...
                countries[row[0]] = {name: lin_interp([float(n.replace(',', '')) / 100 if n != '' else 0 for n in row[1:-1]] if len(row[1:-1]) == 12 else [float(n.replace(',', '')) / 100 if n != '' else 0 for n in row[1:-1]] + [0])}
            line_count += 1
...
if __name__ == '__main__':
    ...
    for raw_file_name in raw_file_names:
        if 'events' in raw_file_name:
            ...
            read_csv_12_year(os.path.join(raw_dir, raw_file_name), processed_dir)
